const card = post => {
    return `
        <div style="border: 1px solid grey; margin: 24px;">
            <div>
                <span>${post.title}</span>
                <p style="white-space: pre-line;">${post.text}</p>
                <small>${new Date(post.date)}</small>
            </div>
            <div>
                <button class='js-remove' data-id="${post._id}">
                    Delete
                </button>
            </div>
        </div>
...
function renderPosts (_posts = []) {
    ...
    if (_posts.length > 0) {
        ...
        $posts.innerHTML = `<div>Постов пока нет</div>`
    }
}
